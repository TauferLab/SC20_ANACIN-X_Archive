...
pnmpi_conf=${anacin_x_root}/anacin-x/pnmpi/configs/timer_dumpi.conf

LD_PRELOAD=${pnmpi} PNMPI_LIB_PATH=${pnmpi_lib_path} PNMPI_CONF=${pnmpi_conf} srun -N${n_nodes} -n${n_procs} ${mini_amr_bin} \
    --num_refine 4 \
    --max_blocks 8000 \
    --init_x 1 \
    --init_y 1 \
