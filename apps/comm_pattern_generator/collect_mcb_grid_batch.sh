...
if [ ${system} == "quartz" ] || [ ${system} == "catalyst" ]; then
    if [ -z "${perturb_msg_order}" ]; then
        results_root="/p/lscratchh/chapp1/thesis_results/comm_patterns/mcb_grid/system_${system}/"
    else
        results_root="/p/lscratchh/chapp1/thesis_results/comm_patterns/mcb_grid_perturbed/system_${system}/"
    fi
fi
...
config_dir=${anacin_x_root}/apps/comm_pattern_generator/config/mcb_grid/
...
do
    ...
    do
        ...
        do
            echo "Launching jobs for: proc. placement = ${proc_placement}, # procs. = ${n_procs}, ND% = ${ndp}"
            runs_root=${results_root}/n_procs_${n_procs}/proc_placement_${proc_placement}/ndp_${ndp}/
            ...
            do
                ...
                config=${config_dir}/mcb_grid_interleaved_ndp_${ndp}.json
                ...
                if [[ -a ${job_script_trace_pack_procs} && -a ${job_script_trace_spread_procs} && -a ${app} && -a ${config} && -a ${job_script_build_graph} && -a ${dumpi_to_graph_bin} && -a ${dumpi_to_graph_config} ]]; then
                    sbatch -N1 ${job_script_write_run_params} --pattern "mcb_grid" --n_proc ${n_procs} --proc_placement ${proc_placement} --nd_percentage_msg ${ndp}
