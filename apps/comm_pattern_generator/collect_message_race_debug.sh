...
if [ ${system} == "quartz" ] || [ ${system} == "catalyst" ]; then
    if [ -z "${perturb_msg_order}" ]; then
        results_root="/p/lscratchh/chapp1/thesis_results/comm_patterns/message_race_iters_10/system_${system}/"
    else
        results_root="/p/lscratchh/chapp1/thesis_results/comm_patterns/message_race_perturbed_iters_10/system_${system}/"
    fi
fi
...
config_dir=${anacin_x_root}/apps/comm_pattern_generator/config/message_race/
        ...
        do
            ...
            do
                ...
                do
                    ...
                    config=${config_dir}/message_race_ndp_${ndp}_iters_10_msg_size_${msg_size}.json
                    # Check that all required files exist
                    if [[ -a ${job_script_write_run_params} && -a ${job_script_trace_pack_procs} && -a ${job_script_trace_spread_procs} && -a ${app} && -a ${config} && -a ${job_script_build_graph} && -a ${dumpi_to_graph_bin} && -a ${dumpi_to_graph_config} ]]; then
                        # Write run params
                        ${job_script_write_run_params} --pattern "message_race" --n_proc ${n_procs} --proc_placement ${proc_placement} --msg_size ${msg_size} --nd_percentage_msg ${ndp}
                        ...
                        if [ ${proc_placement} == "pack" ]; then
                            ...
                            ${job_script_trace_pack_procs} ${n_procs} ${app} ${config} >& trace.out
                        elif [ ${proc_placement} == "spread" ]; then
                            n_nodes_trace=${n_procs}
                            ${job_script_trace_spread_procs} ${n_procs} ${app} ${config} >&  trace.out
                        fi
                        # Build event graph
                        n_nodes_build_graph=$(echo "(${n_procs} + ${n_procs_per_node} - 1)/${n_procs_per_node}" | bc)
                        ${job_script_build_graph} ${n_procs} ${dumpi_to_graph_bin} ${dumpi_to_graph_config} ${run_dir} >& build_graph.out
                    fi
                done # runs
